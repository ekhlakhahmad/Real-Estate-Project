import React from "react";

const Gallery = () => {
	return (
		<div className="flex items-center flex-col my-10 justify-center">
			<h3 className="p-10 text-center text-6xl text-blue-600 font-poppins">
				Gallery
			</h3>
			<div className="w-9/12 flex justify-evenly flex-wrap gap-10 items-center">
				<img
					className="h-[265px] w-[350px] rounded-lg cursor-pointer hover:scale-105 duration-1000"
					src="https://images.unsplash.com/photo-1582063289852-62e3ba2747f8?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mjh8fGhvdXNlfGVufDB8fDB8fHww"
					alt="image"
				/>
				<img
					className="h-[265px] w-[350px] rounded-lg cursor-pointer hover:scale-105 duration-1000"
					src="https://images.unsplash.com/photo-1505691723518-36a5ac3be353?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8aG9tZXxlbnwwfHwwfHx8MA%3D%3D"
					alt="image"
				/>
				<img
					className="h-[265px] w-[350px] rounded-lg cursor-pointer hover:scale-105 duration-1000"
					src="https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8OHx8aG91c2V8ZW58MHx8MHx8fDA%3D"
					alt="image"
				/>
				<img
					className="h-[265px] w-[350px] rounded-lg cursor-pointer hover:scale-105 duration-1000"
					src="https://images.unsplash.com/photo-1576941089067-2de3c901e126?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTZ8fGhvdXNlfGVufDB8fDB8fHww"
					alt="image"
				/>
				<img
					className="h-[265px] w-[350px] rounded-lg cursor-pointer hover:scale-105 duration-1000"
					src="https://plus.unsplash.com/premium_photo-1661915661139-5b6a4e4a6fcc?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTd8fGhvdXNlfGVufDB8fDB8fHww"
					alt="image"
				/>
				<img
					className="h-[265px] w-[350px] rounded-lg cursor-pointer hover:scale-105 duration-1000"
					src="https://images.unsplash.com/photo-1575517111478-7f6afd0973db?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjJ8fGhvdXNlfGVufDB8fDB8fHww"
					alt="image"
				/>
			</div>
		</div>
	);
};

export default Gallery;
